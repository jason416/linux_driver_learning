...
static int led_probe(struct platform_device *pdev)
{
	...
	/* get ID and PIN in dts of led device */
	ret = of_property_read_u32(pdev->dev.of_node, "id", &pdev->id);
	if(ret)
		goto id_err;
	ret = of_property_read_u32(pdev->dev.of_node, "pin", &pin);
	if(ret)
		goto pin_err;

	DEBUG_INFO("dev_name: %s, id = %d, pin = %d\n", pdev->name, pdev->id, pin);
...
static const struct of_device_id led_of_matches[] = {
	{ .compatible = "itop4412, led", },
	{ /* null terminated */	}
};
MODULE_DEVICE_TABLE(of, led_of_matches);

struct platform_driver led_drv = {
	.driver = {
		.name = LED_DEV_NAME,	/* must match to its platform_device name */
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(led_of_matches),/* match table of device tree */
	},
	.probe = led_probe,
	.remove = led_remove,
};
...
MODULE_DESCRIPTION("A simple character device driver for LED on itop4412 board, use dtb");
